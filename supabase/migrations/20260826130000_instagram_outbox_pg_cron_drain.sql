-- Instagram outbox recovery scheduler (Supabase pg_cron + pg_net + Vault).
-- Vercel Hobby cron is once daily; this job is the independent safety net.
-- Next.js after() remains the immediate delivery path.
--
-- The Vercel drain handler must finish under this 45-second pg_net timeout
-- (Graph batch 2 + email batch 2 with an explicit ~35s deadline).
--
...
--   instagram_outbox_drain_url
--   instagram_outbox_drain_secret
--
-- Manual Vault setup (run in the SQL editor; replace placeholders):
--
--   select vault.create_secret(
--     'https://YOUR_VERCEL_HOST/api/internal/meta/instagram-outbox/drain',
--     'instagram_outbox_drain_url',
--     'Protected Instagram outbox drain URL'
--   );
--
--   select vault.create_secret(
--     'YOUR_INSTAGRAM_OUTBOX_DRAIN_SECRET',
--     'instagram_outbox_drain_secret',
--     'Bearer token for Instagram outbox drain'
--   );
--
-- Never put the real secret in a migration, repository, or logs.
-- Never log decrypted Vault values.
...
BEGIN
  FOR existing_jobid IN
    SELECT jobid
    FROM cron.job
    WHERE jobname = 'instagram-outbox-drain'
  LOOP
  ...
  FROM (
    SELECT
      (
        ...
        WHERE name = 'instagram_outbox_drain_url'
        LIMIT 1
      ) AS drain_url,
      (
        SELECT decrypted_secret
        FROM vault.decrypted_secrets
        WHERE name = 'instagram_outbox_drain_secret'
        LIMIT 1
      ) AS drain_secret
  ) secrets
