-- WATI outbox recovery scheduler (Supabase pg_cron + pg_net + Vault).
...
-- (WATI batch 2 with an explicit ~26s send budget).
--
...
--     'https://YOUR_VERCEL_HOST/api/internal/wati/outbox/drain',
--     'wati_outbox_drain_url',
--     'Protected WATI outbox drain URL'
--   );
--
--   select vault.create_secret(
--     'YOUR_WATI_OUTBOX_DRAIN_SECRET',
--     'wati_outbox_drain_secret',
--     'Bearer token for WATI outbox drain'
--   );
--
...
BEGIN
  FOR existing_jobid IN
    SELECT jobid
    FROM cron.job
    WHERE jobname = 'wati-outbox-drain'
  LOOP
  ...
  FROM (
    SELECT
      (
        ...
        WHERE name = 'wati_outbox_drain_url'
        LIMIT 1
      ) AS drain_url,
      (
        SELECT decrypted_secret
        FROM vault.decrypted_secrets
        WHERE name = 'wati_outbox_drain_secret'
        LIMIT 1
      ) AS drain_secret
  ) secrets
